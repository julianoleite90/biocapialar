...
export function useUTMTracking() {
  ...
  const addUTMsToURL = (baseURL: string): string => {
    ...
    try {
      ...
      // Preservar parâmetros existentes (como src=pagina2)
      // Só adicionar UTMs se eles não existirem na URL
      ...
      // Adicionar timestamp para tracking (só se não existir)
      if (!url.searchParams.has('timestamp')) {
        url.searchParams.set('timestamp', Date.now().toString())
      }
      
      // Adicionar identificador da landing page (só se não existir)
      if (!url.searchParams.has('lp_source')) {
        url.searchParams.set('lp_source', 'landing_page_phynamax_2')
      }
